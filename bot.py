...
def _handle_image(sender: str, message: dict):
    ...
    try:
        ...
        canonical  = _get_canonical_chofer(chofer_raw)
        if chofer_raw and canonical is None:
            log.warning(f"Chofer no registrado '{chofer_raw}' rechazado para {_mask(sender)}")
